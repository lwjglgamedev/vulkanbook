...
public class DescAllocator {
    ...
    private static Map<Integer, Integer> createDescLimits(PhysDevice physDevice) {
        ...
        descLimits.put(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, (int)Math.min(maxDescs, Integer.toUnsignedLong(limits.maxPerStageDescriptorStorageImages())));
        descLimits.put(VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, (int)Math.min(maxDescs,
                Integer.toUnsignedLong(physDevice.getAccelerationProperties().maxDescriptorSetAccelerationStructures())));
        return descLimits;
    }
